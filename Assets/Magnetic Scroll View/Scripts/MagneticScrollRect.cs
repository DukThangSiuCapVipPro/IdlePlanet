...
namespace MagneticScrollView
{
    ...
    public class MagneticScrollRect : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        [Tooltip ("Reference to the RectTransform component of the Detection Rectangle.")]
        ...
        [Tooltip ("The amount of gap for the last element")]
        [SerializeField, Range (0, 100)] private int scrollLimitGap = 0;
        ...
        [SerializeField, Range (0f, 1f)] private float m_dragDelay = 0.2f;
        ...
        private int ScrollEndingGap
        {
            get
            {
                if (elements == null || m_infiniteScrolling)
                    return 0;
                else
                    return Mathf.Clamp (scrollLimitGap, 0, elements.Length - 1);
            }
        }
        ...
        public int CurrentSelectedIndex
        {
            get
            {
                ...
                //     return (int)Mathf.Repeat (m_currentSelected, elements.Length);
                // else
            ...
            {
                if (elements != null && elements.Length > 0)
                    m_currentSelected = (int)Mathf.Repeat (value, elements.Length);
                else
                    m_currentSelected = 0;
            }
        }
        ...
        void Awake ()
        {
            ...
            indexTableManager = gameObject.GetComponentInChildren<IndexTableManager> ();
        ...
        private void Update ()
        {
            ...
            if (viewport != null)
            {
                if ((!Application.isPlaying || (Application.isPlaying && m_autoArranging)) && ElementsChanged ())
                {
                    AssignElements ();
                    ArrangeElements ();
                }

                if (!Application.isPlaying || (Application.isPlaying && m_autoArranging))
                {
                    for (int i = 0; i < elements.Length; i++)
                    {
                        //Debug.Log (elements [i].hasChanged);
        ...
        private void SetupStartingSettings ()
        {
            ...
            if (viewport != null && ActiveChildCount (viewport) > 0)
            {
                //isArranging = false;
                AssignElements ();
                ResetScroll ();
        ...
        private List<GameObject> ActiveChildren (Transform parent)
        {
        ...
        {
            var activeChildrenList = ActiveChildren (viewport);
            if (elements.Length == activeChildrenList.Count)
            {
                for (int i = 0; i < activeChildrenList.Count; i++)
                {
                    if (activeChildrenList [i] != null && elements [i] != null)
                    {
                        if (activeChildrenList [i].gameObject != elements [i].gameObject)
                            return true;
                    }
                    else
                    {
        ...
        public void AssignElements ()
        {
            ...
            if (viewport != null)
            {
                int count = ActiveChildCount (viewport);
                elements = new RectTransform [count];
                elementAngleOffset = new float [count];

                List<RectTransform> elementList = new List<RectTransform> ();
                ...
                for (int i = 0; i < elements.Length + 1; i++)
                {
                    ...
                    if (gameObject != null)
                    {
                        ...
                        if (Application.isPlaying)
                            changeCheck.enabled = m_autoArranging;
                        else
        ...
        /// </summary>
        /// <param name="resize">Whether the elements should be resized or not, true by default.</param>
        public void ArrangeElements (bool resize = true)
        {
            if (resize)
                ResizeElements ();
            AlignElements ();
        ...
        /// <summary>
        /// Starts the auto arranging mode. This will detect any change made to elements or to the viewport, forcing it to re-organize all elements. Some settings changes may cause the auto-arranging. It is recommended only while adding, removing or changing elements, due to performance hit.
        /// </summary>
        public void StartAutoArranging ()
        {
            ...
            if (viewport != null)
            {
                ChangeCheckCallback [] changeCheckList = viewport.GetComponentsInChildren<ChangeCheckCallback> ();
                foreach (ChangeCheckCallback changeCheck in changeCheckList)
                    changeCheck.enabled = true;
            }
        }

        /// <summary>
        /// Stops the auto arranging mode. This will prevent the elements of being automatically re-organized, even if elements, viewport or settings has been changed. Improve performance by stopping the AutoArranging.
        /// </summary>
        ...
        {
            if (elementAngleOffset == null || elementAngleOffset.Length < elements.Length - 1)
                return 0;
        ...
        private IEnumerator InertiaCoroutine ()
        {
            ...
            while (Mathf.Abs (singleAxisSpeed) > 0f)
            {
                ...
                {
                    int nearestElement = FindNearestElement ();
                    CurrentSelectedIndex = nearestElement;
                    InvokeOnSelectionChangeEvent ();
        ...
        private IEnumerator ScrollCoroutine (bool intertiaEnd)
        {
            if (!isScrolling && elements.Length > 0)
            {
                ...
                //Debug.Log (elementAngleOffset);
                float indexAngle = -elementAngleOffset [m_currentSelected];
                if (m_layoutMode == LayoutMode.Linear && m_snapMode == SnapMode.Swipe)
                    indexAngle = -elementAngleOffset [Mathf.Clamp (m_currentSelected, 0, Mathf.Clamp (elements.Length - 1 - scrollLimitGap, 0, int.MaxValue))];
        ...
        public void ResetScroll ()
        {
            ...
            AlignElements ();
            InstantScrollTo (CurrentSelectedIndex);
            //SetScrollingAngle (0);
        ...
        {
            //Debug.Log (isScrolling);
            RectTransform rectArea = (scrollRect == null) ? this.viewport : scrollRect;
            if (ped.button != PointerEventData.InputButton.Left || !RectTransformUtility.RectangleContainsScreenPoint (rectArea, ped.position, ped.pressEventCamera))
                return;
        ...
        public void OnBeginDrag (PointerEventData ped)
        {
            ...
            pointerDown = false;
            if ((rectArea != null && !RectTransformUtility.ScreenPointToLocalPointInRectangle (rectArea, ped.position, ped.pressEventCamera, out startLocalCursor)) ||
                !RectTransformUtility.RectangleContainsScreenPoint (rectArea, ped.position, ped.pressEventCamera))
                return;
        ...
        public void OnEndDrag (PointerEventData ped)
        {
            ...
            if (rectArea != null && !RectTransformUtility.ScreenPointToLocalPointInRectangle (rectArea, ped.position, ped.pressEventCamera, out lastLocalCursor))
                return;
            ...
            ViewportCG.blocksRaycasts = (m_snapMode.IsEqual (SnapMode.None)) ? true : false;
        }
        ...
        public void OnDrag (PointerEventData ped)
        {
            ...
            if (rectArea != null && !isScrolling)
            {
                if (!RectTransformUtility.ScreenPointToLocalPointInRectangle (rectArea, ped.position, ped.pressEventCamera, out lastLocalCursor))
                    return;
        ...
        private void InvokeOnSelectionChangeEvent ()
        {
            if (onSelectionChange != null && elements != null && elements.Length > m_currentSelected && !elements [m_currentSelected].gameObject.Equals (null))
                onSelectionChange.Invoke (elements [m_currentSelected].gameObject);
        }

        private void SetScrollingAngle (float step)
        {
            if (viewport != null && elements != null && elements.Length > 0)
            {
                //Debug.Log (step);
        ...
        {
            if (transformArray [index] == null || elements.Length < 1)
                return;
            ...
            {
                pos.z = 0;
                transformArray [index].localPosition = Vector3.zero;
            }
            ...
            if (m_rotate && m_layoutMode == LayoutMode.Circular)
            {
                if (m_alignment == Alignment.Horizontal)
                    ...
                    transformArray [index].localEulerAngles = Vector3.right * (m_circularFactor * (angle * curve));
            }
            else
            {
            ...
            elements [index].hasChanged = false;
        ...
        private void SetupSwipeDetection ()
        {
            ...
            if (m_swipeDetect != null && m_snapMode.IsEqual (SnapMode.Swipe, SnapMode.Both))
            {
                ...
                m_swipeDetect.scrollRect = (scrollRect == null) ? viewport : scrollRect;
